...
describe('Transactions routes', () => {
    ...
    test('shold to be able to create a new transaction', async () => {
    ...
    test('shold to be able to list all transactions', async () => {
        ...
        const listTransactionsResonse = await request(app.server).get('/transactions').set('Cookie', cookies).expect(200)
    ...
    test('shold to be able to get a specific transaction', async () => {
        const createTransactionResponse = await request(app.server).post('/transactions').send({
            title:'new transaction',
            amount: 5000,
            type: 'credit'
    ...
    test('shold to be able to get the summary', async () => {
        const createTransactionResponse = await request(app.server).post('/transactions').send({
